#!/usr/bin/env Rscript
#markov_chains.R
#This program show you how to generate a DNA sequence using a multinomial 
#model of DNA sequence evolution, and then moves onto a Markov model 
#of DNA sequence evolution, plotting data as we go along
#adapted from from Benjamin Tovar and Avril Coghlan, Chesley Leslin
...
#Count the frequency of each base,dinucleotide, and trinucleotide 
...
# We have to use data frames in ggplot 2, so just covert, and the give column names
    df1 <- as.data.frame(freqNucleotide)
    colnames(df1) <- c("Base", "Base_Proportion")
    p1 <- ggplot(df1, aes(x = Base, y = Base_Proportion, fill=Base )) +
    geom_bar(stat = "identity")
    p1 <- p1 + theme(legend.position="none") +
    ggtitle("Compositional bias of each nucleotide")

    df2 <- as.data.frame(freqDiNucleotide)
    colnames(df2) <- c("Base", "Base_Proportion")
    p2 <- ggplot(df2, aes(x = Base, y = Base_Proportion, fill=Base )) +
    geom_bar(stat = "identity")
    p2 <- p2 + theme(legend.position="none") +
    ggtitle("Compositional bias of each dinucleotide")

    df3 <- as.data.frame(freqTriNucleotide)
    colnames(df3) <- c("Base", "Base_Proportion")
    p3 <- ggplot(df3, aes(x = Base, y = Base_Proportion, fill=Base )) +
    geom_bar(stat = "identity")
...
generateFirstOrderhmmseq <- function(lengthSeq, nucleotides, initialProb, states, transitionmatrix, emissionmatrix){
  ...
  #    emissionmatrix   = matrix that stores the probability distribution of emissions
    ...
    mystates        <- character()             # Create a vector for storing the state that each position in the new sequence was generated by choose the state for the first position in the sequence:
